...
//  def jsonToTxt() = {
//    new File(Config.dataPath +"nyt/confidence").listFiles().filter(_.getName.startsWith("nyt_")).foreach(f => {
//      saveAsText(by("confidence/" + f.getName).map(Article.toStringSerialized),f.getName.substring(0, f.getName.length-5))
//    })
//  }
